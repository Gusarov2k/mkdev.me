RSpec.describe MovieCollection do
  ...
  describe '.new' do
    ...
    context 'when all good' do
      ...
      it 'creates Array of Movie' do
        expect(subject.instance_variable_get(:@movies).any? { |e| e.is_a?(Movie) }).to be_truthy
      end
  ...
  describe '#sort_by' do
    subject { movie_collection.sort_by(arg) }
  ...
  describe '#select' do
    context 'when call with hash' do
      subject { movie_collection.select(country: 'USA') }
    ...
    context 'when call with block' do
      subject { movie_collection.select { |m| m.genre.include?('Comedy') } }
